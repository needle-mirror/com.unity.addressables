...
namespace UnityEngine.AddressableAssets.ResourceProviders.Tests
{
    ...
    public class GroupRootAssetEntryProviderIntegrationTests : AddressablesTestFixture
    {
        ...
        private string k_TextureGuidFile
        {
            get
            {
                return $"{GetGeneratedAssetsPath()}/GroupRootAssetEntryProviderTests_TextureGuid.txt";
            }
        }
...
#pragma warning restore 618
        ...
        protected override void RunBuilder(AddressableAssetSettings settings)
        {
            ...
            {
                Debug.LogError($"GroupRootAssetEntryProviderIntegrationTests: RunBuilder failed: {ex}");
        ...
        protected override IEnumerator InitAddressables()
        {
            ...
            if (op.Status != AsyncOperationStatus.Succeeded)
            {
                ...
                Debug.LogError(
                    $"GroupRootAssetEntryProviderIntegrationTests: InitializeAsync failed, status={op.Status}. {details}");
        ...
        {
            // This test validates the code path in GroupRootAssetEntryProvider.Provide where
            // info.loadable.isLoaded is true. To trigger this:
            // 1. Load asset (Loadable.Load() is called)
            // 2. Clear operation cache (forces new operation on next load)
            // 3. Load same asset (Provide() called again, but Loadable is already loaded)

            // First load - this will call Loadable.Load()
            var handle = m_Addressables.LoadAssetAsync<GameObject>(k_PrefabKey);
            yield return handle;
            ...
            // (normally the second load would just return the cached operation)
            ...
            // Second load - this will call Provide() again, and info.loadable.isLoaded should be true
        ...
        public IEnumerator LoadAsset_WhenAlreadyLoaded_DoesNotThrowError()
        {
            ...
            // Second load of same asset (Loadable should already be loaded)
            // This tests the fix in GroupRootAssetEntryProvider.cs:53-56
            var handle2 = m_Addressables.LoadAssetAsync<GameObject>(k_PrefabKey);
            yield return handle2;
