...
namespace UnityEditor.AddressableAssets.BuildReportVisualizer
{
    ...
    class LabelsContentView : ContentView
    {
        ...
        private void BuildTree()
        {
            ...
            LoadTree<LabelsViewBuildReportItem>(builder =>
            {
                int id = 0;
                foreach (var item in m_TreeItems)
                {
                    ...
                    {
                        var assetReportItem = new LabelsViewBuildReportAsset(asset);
                        var childrenOfAsset = GenerateChildrenOfAsset(asset, ref id, true);
                        var tvid = new TreeViewItemData<LabelsViewBuildReportItem>(++id, assetReportItem, childrenOfAsset);
                        dataHashToReportItem.TryAdd(BuildReportUtility.ComputeDataHash(label.Name, asset.AddressableName), new TreeDataReportItem(id, tvid.data));
                        assetsUnderLabel.Add(tvid);
                    }

                    var rootItem = new TreeViewItemData<LabelsViewBuildReportItem>(++id, label, assetsUnderLabel);
                    dataHashToReportItem.TryAdd(BuildReportUtility.ComputeDataHash(label.Name), new TreeDataReportItem(id, rootItem.data));
        ...
        {
            await PerformSearchAsyncImpl<LabelsViewBuildReportItem>(searchValue, cancellationToken);
        }
    }
}
