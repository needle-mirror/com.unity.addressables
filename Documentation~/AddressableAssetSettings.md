...
| __Extract TypeTree Data__| Places TypeTree data in a separate file, which reduces the build size of Addressables. **Important**: This changes all AssetBundles in already deployed projects, so only use it in new projects. For more information, refer to [TypeTree management](memory-assetbundles.md#typetree-management).|
