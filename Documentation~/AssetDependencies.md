...
- **Addressable assets**: Depending on how you configure the Addressables settings in your project you can either:
    ...
    - Keep Addressable assets external to the build as remote assets hosted on a server and downloaded when they're needed. You can update remote assets independently from the application itself, although remote assets can't include code, so you can only change assets and serialized data. This workflow only applies if you're using the [AssetBundle system](content-build-systems.md).
...
An explicit asset is one you directly add to an [Addressables group](groups-intro.md). Unity packs these into AssetBundles or content directories during a [content build](xref:addressables-builds).

An implicit asset is a dependency that Unity automatically includes. If an explicit asset references other assets:

- **Addressable dependencies**: Unity packs these according to their group settings (same or different AssetBundle or content directory).
...
>[!NOTE]
>Asset duplication only applies to [AssetBundle builds](content-build-systems.md). When you build with content directories, the build system automatically deduplicates content, so a shared non-Addressable asset is included only once and this duplication doesn't happen.
...
* **FBX files**: The main object is the root GameObject of the hierarchy. Each mesh and material in the FBX file is a subasset.
...
You can use `AssetReference` instances that point to a subasset to load just that subasset at runtime. However, AssetReference only works to reference explicit assets and Unity builds the entire content of the asset, not just the objects required by the subasset. This can be inefficient if the asset contains large objects that aren't required at runtime.

You can use the following approaches to reference subassets in your project:

 * **Use an AssetReference that references a subasset**: This references the subasset correctly, but includes the entire asset in the content build.
 * **Use an AssetReference that references an intermediary asset**: Only includes the part of the asset that the subasset needs. This requires an extra ScriptableObject to reference the subasset.

The following examples illustrate those two approaches in the case of a mesh inside an FBX file. Both examples use an AssetReference in a separate AssetBundle to load the mesh on demand. However this approach also applies if you're using content directories.

#### Direct subasset reference

This example shows the common pattern for using AssetReference. A `MeshLoader` ScriptableObject has an AssetReference that directly points to the Unity object you want to load dynamically.
...
This example shows how to include only a single mesh in the build. This works by creating a `MeshLoaderDirect` ScriptableObject that directly references the mesh subasset. The MeshLoaderDirect asset is explicitly assigned to a group, but the MultiMesh.fbx isn't. Like the direct subasset approach, there's also a MeshLoader.asset in another AssetBundle. However, in this intermediary approach, the AssetReference points to the MeshLoaderDirect asset instead of directly to the mesh.
